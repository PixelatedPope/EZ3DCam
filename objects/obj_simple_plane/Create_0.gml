...
vertex_begin(vertex,objEz3dCam.ezCam.vertexFormat);
#region FRONT  
  
//Top Left Tri
vertex_position_3d(vertex,-_w,0,-_h);
vertex_normal(vertex,0,1,0);
vertex_colour(vertex, c_white, 1);
vertex_texcoord(vertex, _uvs[_l], _uvs[_t]);
		
vertex_position_3d(vertex,_w,0,-_h);
vertex_normal(vertex,0,1,0);
vertex_colour(vertex, c_white, 1);
vertex_texcoord(vertex, _uvs[_r], _uvs[_t]);

vertex_position_3d(vertex,-_w,0,_h);
vertex_normal(vertex,0,1,0);
vertex_colour(vertex, c_white, 1);
vertex_texcoord(vertex, _uvs[_l], _uvs[_b]);

//Bottom Right Tri	
vertex_position_3d(vertex,_w,0,_h);
vertex_normal(vertex,0,1,0);
vertex_colour(vertex, c_white, 1);
vertex_texcoord(vertex, _uvs[_r], _uvs[_b]);
