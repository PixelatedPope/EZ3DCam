function use_ez3d_cam(_mode = EZ3DCam.mode_first_person)
{
	...
	{    
		...
		draw: function()
		{
			...
			//Finally, draw the 3D
			if(__iShouldDrawSurface) draw_surface_ext(__iSurface,camera_get_view_x(_cam),camera_get_view_y(_cam),_scale,_scale,0,c_white,1);
		},
		...
		{
			setOrientation([__iYaw + _difArray[0], __iPitch + _difArray[1], __iRoll + _difArray[2]]);
		},
    
		changePosition: function(_difArray)
		{
			setPosition([__iPosition[V_X] + _difArray[V_X], __iPosition[V_Y] + _difArray[V_Y], __iPosition[V_Z] + _difArray[V_Z]]);
		},
    
		changeTargetPosition: function(_difArray)
		{
			setTargetPosition([__iTarget[V_X] + _difArray[V_X], __iTarget[V_Y] + _difArray[V_Y], __iTarget[V_Z] + _difArray[V_Z]]);
		},
		...
		setTargetPosition: function(_posArray)
		{
			...
			__iHasMoved = true;
			__iTarget = _posArray;
			if (__iMode == EZ3DCam.mode_track_target) __iDistance = getDistanceToPoint(__iTarget);
		},
